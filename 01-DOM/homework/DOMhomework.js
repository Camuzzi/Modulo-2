...
const spanId = document.querySelector('#createdBy');

spanId.innerHTML = "Aplicación creada por: Agustin";
...
function buildToDos(toDos) {
  ...
  var nuevoArray = toDos.map ( function(){
    return buildToDo(toDos[i],i);
    i++;
  } );
  /* 
  2da opción:
  ...
  3era opción:
   var newToDos = toDos.map(e -> buildToDo(e, "todo" + i++))
  */
...
function displayToDos() {
  ...
  var nuevoArray = buildToDos(toDoItems);

  /*for (let i=0; i < nuevoArray-1 ; i++){
    toDoContainer = toDoContainer + nuevoArray[i];
  }
  */
...
var EventList = document.querySelector('#addButton');
    EventList.addEventListener("click",addToDo);
